...
public class DoShopCheckoutAdapter extends RecyclerView.Adapter<DoShopCheckoutAdapter.MyViewHolder> {

    private List<DoShopMerchant> data = new ArrayList<>();
    private Activity context;
    private Fragment fragment;

    public DoShopCheckoutAdapter(Activity context) {
        this.context = context;
    }

    public DoShopCheckoutAdapter(Activity context, Fragment fragment) {
        this.context = context;
...
//                        holder.priceStrike.setVisibility(View.GONE);
            ...
            holder.tvChooseCourier.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
//                    Toast.makeText(context, "Choose courier", Toast.LENGTH_SHORT).show();
//                    if (merchant != null){
//                        Intent intent = new Intent(context, DoShopChooseCourierActivity.class);
//                        intent.putExtra(NYHelper.MERCHANT, merchant.toString());
//                        context.startActivityForResult(intent, 100);
//                    }

                    if (merchant != null && fragment instanceof DoShopCheckoutFragment){
                        ((DoShopCheckoutFragment)fragment).chooseCourrier(merchant);
                    }
                }
            });

            holder.tvChangeCourier.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
//                    Toast.makeText(context, "Change courier", Toast.LENGTH_SHORT).show();
//                    if (merchant != null){
//                        Intent intent = new Intent(context, DoShopChooseCourierActivity.class);
//                        intent.putExtra(NYHelper.MERCHANT, merchant.toString());
//                        context.startActivityForResult(intent, 100);
